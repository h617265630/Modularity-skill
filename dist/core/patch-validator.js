...
export class PatchValidator {
     ...
     * 验证 Feature 补丁
     */
    validateFeature(template, compiled) {
        const errors = [];
        const warnings = [];
        // 1. Schema 验证
     ...
     * 验证多个 features
     */
    validateFeatures(templates, compileds) {
        ...
        for (const table of template.database.tables) {
            ...
            if (existing) {
                ...
                for (const fk of existing.foreign_keys) {
                    const hasFk = table.fields.some(f => f.name === fk.column && f.foreign_key === `${fk.reference_table}.${fk.reference_column}`);
    ...
    // 类型一致性验证
    // ============================================================================
    validateTypeConsistency(template) {
        const errors = [];
        // 检查模型和前端组件的类型对应
        for (const model of template.backend.models) {
            for (const field of model.fields) {
                // 查找前端是否有对应的类型使用
                const frontendUsage = template.frontend.components.some(c => c.props?.some(p => p.type.toLowerCase().includes(field.name.toLowerCase())));
                if (frontendUsage) {
                    // 检查类型映射
    ...
    // 路由冲突检测
    // ============================================================================
    detectRouteConflicts(template) {
        const errors = [];
        const existingApis = this.stateGraph.getAllApis();
        for (const route of template.backend.routes) {
            // 精确匹配检查
            const exactMatch = existingApis.find(api => api.path === route.path && api.method === route.method);
            if (exactMatch) {
                // 不是自身引入的冲突
    ...
    // Diff 摘要生成
    // ============================================================================
    generateDiffSummary(template, compiled) {
        const filesCreated = [];
        const filesModified = [];
        // 后端文件
        ...
        return {
            ...
            total_changes: filesCreated.length +
    ...
    isValidHandlerName(name) {
        return /^[a-z][a-z0-9_]*$/.test(name);
    }
    getPythonToTsType(pythonType) {
        const map = {
            'str': 'string',
            'string': 'string',
            'int': 'number',
