...
export class CodeModifier {
    ...
    async findExistingAuthFiles() {
        const result = { loginForms: [], registerForms: [], authHooks: [], apiServices: [], navbars: [] };
    ...
    async findExistingPostFiles() {
        const result = { postLists: [], postItems: [], postInputs: [], postHooks: [], postApis: [] };
        const searchDirs = [path.join(this.projectPath, 'src'), path.join(this.projectPath, 'frontend', 'src'), path.join(this.projectPath, 'app')].filter(d => fs.existsSync(d));
        for (const dir of searchDirs) {
            await this.scanForPosts(dir, result);
        }
        return result;
    }
    async scanForPosts(dir, result) {
        if (!fs.existsSync(dir))
            return;
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
    ...
    async findExistingCommentFiles() {
        const result = { commentLists: [], commentItems: [], commentInputs: [], commentHooks: [], commentApis: [] };
        const searchDirs = [path.join(this.projectPath, 'src'), path.join(this.projectPath, 'frontend', 'src'), path.join(this.projectPath, 'app')].filter(d => fs.existsSync(d));
        for (const dir of searchDirs) {
            await this.scanForComments(dir, result);
        }
        return result;
    }
    async scanForComments(dir, result) {
        if (!fs.existsSync(dir))
            return;
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
            ...
            if (entry.isDirectory() && !['node_modules', '.git', 'dist', 'build', '.next'].includes(entry.name)) {
                ...
                const baseName = entry.name.toLowerCase();
                if (COMMENT_LIST_PATTERNS.some(p => p.test(baseName)))
                    result.commentLists.push(fullPath);
                if (COMMENT_ITEM_PATTERNS.some(p => p.test(baseName)))
                    result.commentItems.push(fullPath);
                if (COMMENT_INPUT_PATTERNS.some(p => p.test(baseName)))
                    result.commentInputs.push(fullPath);
                if (COMMENT_HOOK_PATTERNS.some(p => p.test(baseName)))
                    result.commentHooks.push(fullPath);
                if (COMMENT_API_PATTERNS.some(p => p.test(baseName)))
                    result.commentApis.push(fullPath);
            }
        }
    }
    // ============================================================================
    ...
    async findExistingLikeFiles() {
        const result = { likeButtons: [], likeCounts: [], likeHooks: [], likeApis: [] };
        const searchDirs = [path.join(this.projectPath, 'src'), path.join(this.projectPath, 'frontend', 'src'), path.join(this.projectPath, 'app')].filter(d => fs.existsSync(d));
        for (const dir of searchDirs) {
            await this.scanForLikes(dir, result);
        }
        return result;
    }
    async scanForLikes(dir, result) {
        if (!fs.existsSync(dir))
            return;
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
            ...
            if (entry.isDirectory() && !['node_modules', '.git', 'dist', 'build', '.next'].includes(entry.name)) {
                ...
                const baseName = entry.name.toLowerCase();
                if (LIKE_BUTTON_PATTERNS.some(p => p.test(baseName)))
                    result.likeButtons.push(fullPath);
                if (LIKE_COUNT_PATTERNS.some(p => p.test(baseName)))
                    result.likeCounts.push(fullPath);
                if (LIKE_HOOK_PATTERNS.some(p => p.test(baseName)))
                    result.likeHooks.push(fullPath);
                if (LIKE_API_PATTERNS.some(p => p.test(baseName)))
                    result.likeApis.push(fullPath);
            }
        }
    }
    // ============================================================================
    ...
    async findExistingFollowFiles() {
        const result = { followButtons: [], followersLists: [], followingLists: [], followHooks: [], followApis: [] };
        const searchDirs = [path.join(this.projectPath, 'src'), path.join(this.projectPath, 'frontend', 'src'), path.join(this.projectPath, 'app')].filter(d => fs.existsSync(d));
        for (const dir of searchDirs) {
            await this.scanForFollows(dir, result);
        }
        return result;
    }
    async scanForFollows(dir, result) {
        if (!fs.existsSync(dir))
            return;
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
            ...
            if (entry.isDirectory() && !['node_modules', '.git', 'dist', 'build', '.next'].includes(entry.name)) {
                ...
                const baseName = entry.name.toLowerCase();
                if (FOLLOW_BUTTON_PATTERNS.some(p => p.test(baseName)))
                    result.followButtons.push(fullPath);
                if (FOLLOWERS_LIST_PATTERNS.some(p => p.test(baseName)))
                    result.followersLists.push(fullPath);
                if (FOLLOWING_LIST_PATTERNS.some(p => p.test(baseName)))
                    result.followingLists.push(fullPath);
                if (FOLLOW_HOOK_PATTERNS.some(p => p.test(baseName)))
                    result.followHooks.push(fullPath);
                if (FOLLOW_API_PATTERNS.some(p => p.test(baseName)))
                    result.followApis.push(fullPath);
            }
        }
    }
    // ============================================================================
    ...
    async findExistingNotificationFiles() {
        const result = { notificationBells: [], notificationLists: [], notificationHooks: [], notificationApis: [] };
        const searchDirs = [path.join(this.projectPath, 'src'), path.join(this.projectPath, 'frontend', 'src'), path.join(this.projectPath, 'app')].filter(d => fs.existsSync(d));
        for (const dir of searchDirs) {
            await this.scanForNotifications(dir, result);
        }
        return result;
    }
    async scanForNotifications(dir, result) {
        if (!fs.existsSync(dir))
            return;
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
            ...
            if (entry.isDirectory() && !['node_modules', '.git', 'dist', 'build', '.next'].includes(entry.name)) {
                ...
                const baseName = entry.name.toLowerCase();
                if (NOTIFICATION_BELL_PATTERNS.some(p => p.test(baseName)))
                    result.notificationBells.push(fullPath);
                if (NOTIFICATION_LIST_PATTERNS.some(p => p.test(baseName)))
                    result.notificationLists.push(fullPath);
                if (NOTIFICATION_HOOK_PATTERNS.some(p => p.test(baseName)))
                    result.notificationHooks.push(fullPath);
                if (NOTIFICATION_API_PATTERNS.some(p => p.test(baseName)))
                    result.notificationApis.push(fullPath);
            }
        }
    }
    // ============================================================================
    ...
    async findExistingMessageFiles() {
        const result = { messageLists: [], messageItems: [], messageHooks: [], messageApis: [] };
        const searchDirs = [path.join(this.projectPath, 'src'), path.join(this.projectPath, 'frontend', 'src'), path.join(this.projectPath, 'app')].filter(d => fs.existsSync(d));
        for (const dir of searchDirs) {
            await this.scanForMessages(dir, result);
        }
        return result;
    }
    async scanForMessages(dir, result) {
        if (!fs.existsSync(dir))
            return;
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
            ...
            if (entry.isDirectory() && !['node_modules', '.git', 'dist', 'build', '.next'].includes(entry.name)) {
                ...
                const baseName = entry.name.toLowerCase();
                if (MESSAGE_LIST_PATTERNS.some(p => p.test(baseName)))
                    result.messageLists.push(fullPath);
                if (MESSAGE_ITEM_PATTERNS.some(p => p.test(baseName)))
                    result.messageItems.push(fullPath);
                if (MESSAGE_HOOK_PATTERNS.some(p => p.test(baseName)))
                    result.messageHooks.push(fullPath);
                if (MESSAGE_API_PATTERNS.some(p => p.test(baseName)))
                    result.messageApis.push(fullPath);
            }
        }
    }
    // ============================================================================
    ...
    async findExistingSearchFiles() {
        const result = { searchBars: [], searchResults: [], searchHooks: [], searchApis: [] };
        const searchDirs = [path.join(this.projectPath, 'src'), path.join(this.projectPath, 'frontend', 'src'), path.join(this.projectPath, 'app')].filter(d => fs.existsSync(d));
        for (const dir of searchDirs) {
            await this.scanForSearch(dir, result);
        }
        return result;
    }
    async scanForSearch(dir, result) {
        if (!fs.existsSync(dir))
            return;
        const entries = fs.readdirSync(dir, { withFileTypes: true });
        for (const entry of entries) {
            ...
            if (entry.isDirectory() && !['node_modules', '.git', 'dist', 'build', '.next'].includes(entry.name)) {
                ...
                const baseName = entry.name.toLowerCase();
                if (SEARCH_BAR_PATTERNS.some(p => p.test(baseName)))
                    result.searchBars.push(fullPath);
                if (SEARCH_RESULTS_PATTERNS.some(p => p.test(baseName)))
                    result.searchResults.push(fullPath);
                if (SEARCH_HOOK_PATTERNS.some(p => p.test(baseName)))
                    result.searchHooks.push(fullPath);
                if (SEARCH_API_PATTERNS.some(p => p.test(baseName)))
                    result.searchApis.push(fullPath);
            }
        }
    }
    // ============================================================================
    ...
    async modifyApiService(filePath, newEndpoints) {
        ...
        const newLines = lines.map((line, i) => {
            ...
            if (newEndpoints.login && /['"`]([^'"`]*(?:login|signin)[^'"`]*)['"`]/.test(line)) {
                const m = line.match(/['"`]([^'"`]*(?:login|signin)[^'"`]*)['"`]/);
                if (m) {
                    changes.push({ type: 'api_endpoint', description: 'Replace login path', before: m[1], after: newEndpoints.login, line_start: ln, line_end: ln });
                    line = line.replace(m[1], newEndpoints.login);
                    modified = true;
                }
            }
            if (newEndpoints.register && /['"`]([^'"`]*(?:register|signup)[^'"`]*)['"`]/.test(line)) {
                const m = line.match(/['"`]([^'"`]*(?:register|signup)[^'"`]*)['"`]/);
                if (m) {
                    changes.push({ type: 'api_endpoint', description: 'Replace register path', before: m[1], after: newEndpoints.register, line_start: ln, line_end: ln });
                    line = line.replace(m[1], newEndpoints.register);
                    modified = true;
                }
            }
            return line;
    ...
    async modifyPostHook(filePath, apiEndpoints) {
        ...
        const newLines = lines.map((line, i) => {
            ...
            if (apiEndpoints.list && /['"`]([^'"`]*(?:posts|post\/list)[^'"`]*)['"`]/.test(line)) {
                ...
                if (m) {
                    changes.push({ type: 'api_endpoint', description: 'Replace posts list path', before: m[1], after: apiEndpoints.list, line_start: ln, line_end: ln });
                    line = line.replace(m[1], apiEndpoints.list);
                    modified = true;
                }
            }
            if (apiEndpoints.create && /['"`]([^'"`]*(?:post\/create|posts\/create)[^'"`]*)['"`]/.test(line)) {
                ...
                if (m) {
                    changes.push({ type: 'api_endpoint', description: 'Replace create post path', before: m[1], after: apiEndpoints.create, line_start: ln, line_end: ln });
                    line = line.replace(m[1], apiEndpoints.create);
                    modified = true;
                }
            }
            return line;
    ...
    async modifyPostApi(filePath, newEndpoints) {
        ...
        const newLines = lines.map((line, i) => {
            ...
            if (newEndpoints.list && /['"`]([^'"`]*(?:posts|post-list)[^'"`]*)['"`]/.test(line)) {
                const m = line.match(/['"`]([^'"`]*(?:posts|post-list)[^'"`]*)['"`]/);
                if (m) {
                    changes.push({ type: 'api_endpoint', description: 'Replace posts list path', before: m[1], after: newEndpoints.list, line_start: ln, line_end: ln });
                    line = line.replace(m[1], newEndpoints.list);
                    modified = true;
                }
            }
            if (newEndpoints.create && /['"`]([^'"`]*(?:create-post|post\/create)[^'"`]*)['"`]/.test(line)) {
                ...
                if (m) {
                    changes.push({ type: 'api_endpoint', description: 'Replace create post path', before: m[1], after: newEndpoints.create, line_start: ln, line_end: ln });
                    line = line.replace(m[1], newEndpoints.create);
                    modified = true;
                }
            }
            return line;
    ...
    // ============================================================================
    async modifyApiFile(filePath, moduleName, apiEndpoints) {
        if (!fs.existsSync(filePath))
            return null;
        const content = fs.readFileSync(filePath, 'utf-8');
        const changes = [];
    ...
    generateModificationReport(modifications) {
        if (modifications.length === 0)
            return 'No files needed modification.';
