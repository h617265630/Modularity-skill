...
export class FrontendAnalyzer {
  private projectStructure: FrontendStructure;
  private options: FrontendAnalysisOptions;
   ...
   */
  async analyzeFeature(featureName: string): Promise<DetectedFrontendCode | null> {
   ...
   * 扫描 hooks 目录
   */
  private async scanHooks(featureName: string): Promise<DetectedHook[]> {
    const hooksDir = this.options.hooks_dir;
    if (!hooksDir || !fs.existsSync(hooksDir)) {
      return [];
    }

    const detectedHooks: DetectedHook[] = [];
    ...
    for (const file of hookFiles) {
      ...
      // 检查这个 hook 是否与功能相关
      if (!this.isRelatedToFeature(hookName, featureName)) continue;
   ...
   * 扫描 components 目录
   */
  private async scanComponents(featureName: string, relatedHooks: DetectedHook[]): Promise<DetectedComponent[]> {
    ...
    for (const file of componentFiles) {
      ...
      // 检查组件名是否与功能相关
      if (!this.isRelatedToFeature(componentName, featureName)) continue;
   ...
   * 扫描 API services 目录
   */
  private async scanApiServices(featureName: string): Promise<DetectedApiService[]> {
    const apiDir = this.options.api_services_dir;
    if (!apiDir || !fs.existsSync(apiDir)) {
      return [];
    }

    const detectedServices: DetectedApiService[] = [];
   ...
   * 从文件内容中提取 hook 名称
   */
  private extractHookName(content: string, filePath: string): string | null {
    // 从函数声明提取
   ...
   * 从 hook 内容中提取 API 端点
   */
  private extractEndpoints(content: string): HookEndpoint[] {
    const endpoints: HookEndpoint[] = [];
    ...
    for (const pattern of API_CALL_PATTERNS) {
      ...
      while ((match = regex.exec(content)) !== null) {
        ...
        endpoints.push({
          method: method as HookEndpoint['method'],
          path: apiPath,
          has_auth: hasAuth,
   ...
   * 提取查询参数
   */
  private extractQueryParams(content: string, callIndex: number): string[] {
    const params: string[] = [];
   ...
   * 提取请求体类型
   */
  private extractBodyType(content: string, callIndex: number): string | null {
   ...
   */
  private extractResponseShape(content: string): ResponseShape | null {
    // 尝试从返回类型注解提取
   ...
   * 提取类型字段
   */
  private extractTypeFields(content: string, typeName: string): TypeField[] {
   ...
   * 查找分页字段
   */
  private findField(content: string, field: string): string | null {
   ...
   * 提取认证头
   */
  private extractAuthHeaders(content: string): string[] {
   ...
   * 提取 service 端点
   */
  private extractServiceEndpoints(content: string): ServiceEndpoint[] {
   ...
   * 从函数名推断 HTTP 方法
   */
  private inferMethodFromFuncName(funcName: string, content: string, index: number): string {
    const lower = funcName.toLowerCase();

    if (lower.startsWith('get') || lower.includes('fetch') || lower.includes('list')) return 'GET';
    if (lower.startsWith('create') || lower.includes('add') || lower.includes('post')) return 'POST';
    if (lower.startsWith('update') || lower.includes('put')) return 'PUT';
    ...
    if (funcBody.includes('.get(')) return 'GET';
    if (funcBody.includes('.post(')) return 'POST';
    if (funcBody.includes('.put(')) return 'PUT';
    if (funcBody.includes('.patch(')) return 'PATCH';
    if (funcBody.includes('.delete(')) return 'DELETE';
   ...
   * 提取 props 类型
   */
  private extractPropsType(content: string): string | null {
   ...
   * 判断名称是否与功能相关
   */
  private isRelatedToFeature(name: string, featureName: string): boolean {
    const lowerName = name.toLowerCase();
    const lowerFeature = featureName.toLowerCase();

    // 直接匹配
    if (lowerName.includes(lowerFeature)) return true;

    // 常见单复数变化
    if (lowerName.includes(this.pluralize(lowerFeature))) return true;
    if (lowerName.includes(this.singularize(lowerFeature))) return true;
   ...
   * 获取目录下所有 .ts/.tsx 文件
   */
  private getTsTsxFiles(dir: string): string[] {
    const files: string[] = [];
 ...
 */
export function matchFrontendToBackend(
  detected: DetectedFrontendCode,
  templateRoutes: Array<{ method: string; path: string }>
): IntegrationMatchResult {
  const matchedEndpoints: MatchedEndpoint[] = [];
  const mismatchedEndpoints: MismatchedEndpoint[] = [];

  // 收集前端所有端点
  const frontendEndpoints = new Map<string, HookEndpoint>();
    ...
    for (const ep of service.endpoints) {
      ...
      if (!frontendEndpoints.has(key)) {
        frontendEndpoints.set(key, {
          method: ep.method as HookEndpoint['method'],
          path: ep.path,
          has_auth: false,
          query_params: [],
 ...
 * 生成后端建议
 */
function generateBackendSuggestions(mismatched: MismatchedEndpoint[]): string[] {
  return mismatched
 ...
 * 生成前端建议
 */
function generateFrontendSuggestions(mismatched: MismatchedEndpoint[]): string[] {
  return mismatched
