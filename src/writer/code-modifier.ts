...
import { DetectedFrontendCode, DetectedHook, DetectedComponent } from '../core/types.js';
...
export class CodeModifier {
   ...
   * 查找需要修改的现有前端文件
   */
  async findExistingAuthFiles(): Promise<{
    loginForms: string[];
    ...
    const result = {
      loginForms: [] as string[],
      registerForms: [] as string[],
      authHooks: [] as string[],
   ...
   * 查找帖子相关的现有前端文件
   */
  async findExistingPostFiles(): Promise<{
    postLists: string[];
    postItems: string[];
    postInputs: string[];
    postHooks: string[];
   ...
   * 扫描目录查找帖子相关文件
   */
  private async scanDirectoryForPosts(dir: string, result: {
    postLists: string[];
    postItems: string[];
    postInputs: string[];
    postHooks: string[];
   ...
   * 扫描目录查找认证相关文件
   */
  private async scanDirectoryForAuth(dir: string, result: {
    loginForms: string[];
    registerForms: string[];
    authHooks: string[];
    apiServices: string[];
   ...
   * 扫描目录查找认证相关文件
   */
  private async scanDirectory(dir: string, result: {
    loginForms: string[];
    registerForms: string[];
    authHooks: string[];
    apiServices: string[];
  ...
  async modifyLoginForm(filePath: string, newApiEndpoint: string): Promise<FileModification | null> {
    ...
    const newLines = lines.map((line, index) => {
      ...
      if (importMatch && this.adapterMappings.length > 0) {
        const adapter = this.adapterMappings.find(m =>
          m.original_endpoint.includes(importMatch[1]) ||
   ...
   * 修改认证 Hook 文件
   */
  async modifyAuthHook(filePath: string, apiEndpoints: {
    login?: string;
    register?: string;
    logout?: string;
    me?: string;
   ...
   * 修改 API Service 文件
   */
  async modifyApiService(filePath: string, newEndpoints: {
    login?: string;
    register?: string;
  }): Promise<FileModification | null> {
    if (!fs.existsSync(filePath)) {
   ...
   */
  async modifyNavbar(filePath: string, options: {
    ...
    if (loginButtonPattern.test(modifiedContent)) {
      ...
      const replacedContent = modifiedContent.replace(
        loginButtonPattern,
        (match) => {
          // 如果已经有条件渲染，不替换
      ...
      if (replacedContent !== modifiedContent) {
        ...
        if (useAuthPattern.test(replacedContent)) {
          const finalContent = replacedContent.replace(
            useAuthPattern,
            (match) => {
   ...
   */
  private async addUserDropdownToNavbar(filePath: string): Promise<FileModification | null> {
    const content = fs.readFileSync(filePath, 'utf-8');
    const lines = content.split('\n');
    const changes: ModificationChange[] = [];
...
`;
   ...
   * 修改帖子 Hook 文件
   */
  async modifyPostHook(filePath: string, apiEndpoints: {
    list?: string;
    detail?: string;
    create?: string;
    update?: string;
   ...
   * 修改帖子 API Service 文件
   */
  async modifyPostApi(filePath: string, newEndpoints: {
    list?: string;
    create?: string;
  }): Promise<FileModification | null> {
    if (!fs.existsSync(filePath)) {
   ...
   * 尝试给文件添加 API 端点配置
   */
  private async addApiEndpointToFile(
    filePath: string,
    endpoint: string,
    type: 'login' | 'register' | 'auth' | 'posts' | 'post' | 'createPost'
  ): Promise<FileModification | null> {
    const content = fs.readFileSync(filePath, 'utf-8');
    const lines = content.split('\n');
    const changes: ModificationChange[] = [];
...
export class AICodeModifier extends CodeModifier {
   ...
   * 智能修改登录/注册页面
   */
  async modifyAuthPages(detected: DetectedFrontendCode): Promise<FileModification[]> {
...
export async function modifyExistingFrontendCode(
  projectPath: string,
...
export async function findAndModifyAuthFiles(
  projectPath: string,
  apiEndpoints: {
    login: string;
    register: string;
