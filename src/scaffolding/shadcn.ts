 ...
 * 模块需要的组件映射
 */
export const MODULE_SHADCN_REQUIREMENTS: Record<string, string[]> = {
  // 评论模块需要的组件
 ...
 * 检测项目是否使用 shadcn/ui
 */
export async function detectShadcn(projectPath: string): Promise<boolean> {
 ...
 */
export async function getInstalledComponents(projectPath: string): Promise<string[]> {
 ...
 */
export function getComponentsForModule(moduleName: string): string[] {
 ...
 * 生成 shadcn/ui 组件导入代码
 */
export function generateShadcnImports(components: string[]): string {
  const imports = components.map(c => {
    const componentName = c.split('-').map(s =>
