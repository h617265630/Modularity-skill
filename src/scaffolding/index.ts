...
export class ProjectScaffolder {
   ...
   */
  private async scaffoldFrontend(): Promise<void> {
    ...
    await fs.promises.writeFile(
      path.join(frontendBase, 'package.json'),
...
`;
    ...
    await fs.promises.writeFile(
      path.join(frontendBase, 'tsconfig.json'),
...
`;
    ...
    // postcss.config.js
    await fs.promises.writeFile(
      path.join(frontendBase, 'postcss.config.js'),
      'module.exports = {\n  plugins: {\n    tailwindcss: {},\n    autoprefixer: {},\n  },\n};\n'
    );
...
`;
    ...
    await fs.promises.writeFile(
      path.join(frontendBase, 'components.json'),
      JSON.stringify(componentsJson, null, 2)
    );

    // lib/utils.ts
    await fs.promises.writeFile(
      path.join(frontendBase, 'src/lib/utils.ts'),
   ...
   */
  private async scaffoldBackend(): Promise<void> {
    ...
    await fs.promises.writeFile(
      path.join(backendBase, 'requirements.txt'),
      requirements.join('\n')
    );
   ...
   * 创建基础页面
   */
  private async createBasePages(): Promise<void> {
   ...
   * 创建用户管理模块
   */
  private async createUserManagement(): Promise<void> {
  ...
  private async createDatabaseConfig(): Promise<void> {
    ...
    // .env.example
    await fs.promises.writeFile(
      path.join(backendBase, '.env.example'),
      '# Database\n' +
    ...
    // alembic.ini
    await fs.promises.writeFile(
      path.join(backendBase, 'alembic.ini'),
      '[alembic]\n' +
