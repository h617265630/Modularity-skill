 ...
 * 为模块生成前端页面
 */
export async function generateModulePages(
 ...
 * 生成模块主页
 */
async function generateModuleHomePage(
  template: FeatureTemplate,
  frontendBase: string
): Promise<void> {
 ...
 * 生成模块布局
 */
async function generateModuleLayout(
  template: FeatureTemplate,
  frontendBase: string
): Promise<void> {
  const modulePath = path.join(frontendBase, template.feature_name);
...
`;
    ...
    await fs.promises.writeFile(
      path.join(modulePath, 'layout.tsx'),
      layoutContent
    );
  }
}
