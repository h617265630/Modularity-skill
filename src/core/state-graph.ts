...
export interface SchemaNode {
  ...
  features: string[];            // 依赖此表的 feature 列表
}
...
export interface ApiNode {
  id: string;                    // 唯一标识 (e.g., "GET /comments/{id}")
  ...
  features: string[];            // 使用此端点的 feature 列表
}
...
export interface FeatureNode {
  id: string;                    // feature 命令 (e.g., "/comment-m")
  name: string;
  description: string;
  version: string;
  installed_at: string;
  dependencies: string[];        // 依赖的其他 features
  schema_nodes: string[];        // 使用的表
  api_nodes: string[];           // 使用的 API
...
export class SystemStateGraph {
  private schema_nodes: Map<string, SchemaNode> = new Map();
  private api_nodes: Map<string, ApiNode> = new Map();
  private feature_nodes: Map<string, FeatureNode> = new Map();
  private edges: FeatureEdge[] = [];
  ...
  addTable(table: {
    ...
    const fields: SchemaField[] = table.fields.map(f => {
    ...
    const indexes: IndexDefinition[] = (table.indexes || []).map(idx => ({
      name: idx.name,
      columns: idx.columns,
   ...
   * 合并已存在的表 schema
   */
  private mergeTableSchema(tableName: string, newFields: any[], featureId: string): void {
  ...
  getSharedTables(): Array<{ table: SchemaNode; features: string[] }> {
    const shared: Array<{ table: SchemaNode; features: string[] }> = [];
  ...
  addFeatureApis(template: FeatureTemplate): ApiNode[] {
    ...
    for (const route of template.backend.routes) {
      const existing = this.getApiByPath(route.method as any, route.path);
   ...
   * 查找路由冲突
   */
  findRouteConflicts(): Array<{ api1: ApiNode; api2: ApiNode; type: 'exact' | 'pattern' }> {
    const conflicts: Array<{ api1: ApiNode; api2: ApiNode; type: 'exact' | 'pattern' }> = [];
  ...
  private pathsConflict(path1: string, path2: string): boolean {
    ...
    for (let i = firstParamPos1; i < segs1.length; i++) {
      ...
      if (adjustedI >= segs2.length) break;
   ...
   * 推断 feature 依赖
   */
  private inferDependencies(template: FeatureTemplate): string[] {
    const deps: string[] = [];
   ...
   * 检测循环依赖
   */
  detectCircularDeps(): Array<{ feature: string; path: string[] }> {
    const cycles: Array<{ feature: string; path: string[] }> = [];
    const visited = new Set<string>();
  ...
  topologicalSort(): string[] | null {
    ...
    const dfs = (nodeId: string): boolean => {
      if (temp.has(nodeId)) return false; // 循环依赖
