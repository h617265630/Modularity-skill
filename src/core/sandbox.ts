...
export class ExecutionSandbox {
  ...
  private changeHistory: SandboxResult[] = [];

  constructor(
    config: Partial<SandboxConfig> = {},
    stateGraph?: SystemStateGraph
  ) {
    this.config = {
  ...
  async execute(
    ...
    if (!validation.valid && this.config.mode === 'production') {
      return this.createFailureResult(
        template.command,
        'Validation failed',
        validation.errors.map(e => `[${e.type}] ${e.location}: ${e.message}`)
      );
    }
    ...
    if (this.config.mode === 'dry-run') {
      ...
      if (!this.config.allow_production_write) {
        return this.createFailureResult(
          ...
          ['Set allow_production_write=true to enable']
        );
      }
   ...
   * 应用所有暂存的修改
   */
  async applyStaged(featureId: string): Promise<{
    success: boolean;
    applied: number;
    failed: string[];
  }> {
   ...
   * 前置检查
   */
  private preExecuteCheck(template: FeatureTemplate): {
    canExecute: boolean;
    reason?: string;
    details?: string[];
  } {
   ...
   * 模拟暂存
   */
  private simulateStaging(template: FeatureTemplate, compiled: CompiledFeature): string[] {
   ...
   * 获取要创建的文件列表
   */
  private getFilesToCreate(template: FeatureTemplate): string[] {
    const files: string[] = [];
   ...
   * 获取要修改的文件列表
   */
  private getFilesToModify(template: FeatureTemplate): string[] {
    const files: string[] = [];
   ...
   * 获取文件类型
   */
  private getFileType(filePath: string): 'backend' | 'frontend' | 'database' {
