...
export class Verifier {
  ...
  constructor(options: VerifyOptions = {}, language: 'python' | 'typescript' = 'python') {
   ...
   * 验证模块
   */
  async verify(template: FeatureTemplate, code: GeneratedCode): Promise<VerificationResult> {
   ...
   * 运行 Lint 检查
   */
  private async runLint(code: GeneratedCode, language: 'python' | 'typescript'): Promise<LintResult> {
   ...
   * 运行类型检查
   */
  private async runTypeCheck(code: GeneratedCode, language: 'python' | 'typescript'): Promise<TypeCheckResult> {
   ...
   * 检查契约
   */
  private async checkContracts(template: FeatureTemplate, code: GeneratedCode): Promise<ContractResult> {
    const routes = template.backend?.routes || [];
    const errors: { route: string; method: string; expected_status: number; actual_status?: number; expected_schema?: boolean; actual_schema?: boolean; message: string }[] = [];
   ...
   * 检查集成
   */
  private async checkIntegration(template: FeatureTemplate, code: GeneratedCode): Promise<IntegrationResult> {
    const integration = template.integration || { steps: [] };
    const errors: { dependency: string; type: 'missing' | 'version_mismatch' | 'api_incompatible'; message: string }[] = [];
   ...
   * 计算验证是否成功
   */
  private calculateSuccess(result: VerificationResult): boolean {
   ...
   * 转换测试错误为验证错误
   */
  private convertTestErrors(testErrors: { name: string; message: string }[]): VerificationError[] {
